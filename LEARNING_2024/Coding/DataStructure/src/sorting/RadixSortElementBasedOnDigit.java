...
public class RadixSortElementBasedOnDigit {
	public static void bucketSort(int[] arr, int k){
		// TC=(O(d*(n+k))  d=range of digits 
	...
	public static void main(String[] args){
		...
		// NOW in Radix sort we need to call bucket sort digit's place one by one from 0 to 
		// MSB of max number
		int max=Integer.MIN_VALUE;
		for(int i=0;i<arr.length;i++){
			max=Math.max(max, arr[i]);
		}
		int digit= (int)(Math.log(max)/Math.log(10))+1;
