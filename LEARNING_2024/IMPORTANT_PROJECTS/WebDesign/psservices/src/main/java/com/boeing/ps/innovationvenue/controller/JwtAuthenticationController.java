...
public class JwtAuthenticationController {
    ...
    public ResponseEntity<?> createAuthenticationToken(@RequestBody JwtRequest authenticationRequest) throws Exception {
        ...
        String bemsId = authenticationRequest.getBemsId();
        String emailId = authenticationRequest.getEmailId();

        if(getInfo(bemsId)){
            final String token = jwtTokenUtil.generateToken(new org.springframework.security.core.userdetails.User(bemsId, emailId+"~"+getRoleDesc(bemsId),
                    new ArrayList<>()));
